...
class CartController extends Controller
{
    ...
    public function updateQuantity(Request $request, $productId)
    {
        $request->validate(
            [
                'quantity' => 'required|integer|min:value',
            ]
        );
        $carts = Cart::where('product_id', $productId)->where('user_id', Auth::user()->id)->get();
        foreach($carts as $cart){
            $cart->quantity = $request->quantity;
            $cart->save();
        }
        return redirect()->route('cart.show', Auth::user()->id)->with(['success' => 'Quantity updated successfully']);
    }
