...
class CheckoutController extends Controller
{
    ...
    public function showCheckoutedOrders() {
    $orderRecipients = OrderRecipient::with('orders.product')->where('	order_maker_id ')->get();
    $orderRecipients->each(function ($recipient) {
        $recipient->orders->subtotal = PriceController::subTotal($recipient->orders);
    });

    return view('orders', compact('orderRecipients'));
    }
