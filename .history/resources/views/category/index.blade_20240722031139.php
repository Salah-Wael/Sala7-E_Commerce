                    ...
                    <div class="col-lg-4 col-md-6 text-center">
                        <div class="single-product-item">
                            ...
                            @if (Auth::check() && Auth::user()->role == 'admin')
                                <center>
                                    <a href="{{ route('category.edit', $category->name) }}" class="cart-btn"><i class="fas fa-edit"></i> Edit</a>
        
                                    <a href="{{ route('category.delete', $category->name) }}"
                                        onclick="event.preventDefault();
                                        document.getElementById('delete-category-form').submit();" class="cart-btn">
                                    <i class="fas fas fa-trash"></i>
                                    {{ __('Delete') }}
                                    </a>
                                    <form id="delete-category-form" action="{{ route('category.delete', $category->name) }}" method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </center>
