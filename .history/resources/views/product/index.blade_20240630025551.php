                    ...
                    <div class="col-lg-4 col-md-6 text-center {{ str_replace(' ', '', $product->category_name) }}">
                        <div class="single-product-item">
                            ...
                            <a href="{{ route('cart.store', $product->id) }}" 
                                ...
                                document.getElementById('cart-form').submit();" class="cart-btn">
                                <i class="fas fa-shopping-cart"></i>
                                {{ __('Add to Cart') }}
                            </a>
                            <form id="cart-form" action="{{ route('cart.store', $product->id) }}" method="POST" class="d-none">
                                @csrf
                            </form>
                            
                            @if (Auth::check() && Auth::user()->role == 'admin')
                                <a href="{{ route('product.delete', $product->id) }}" class="cart-btn"><i class="fas fa-edit"></i> Edit</a>
                                <a href="{{ route('cart.store', $product->id) }}" 
                                    onclick="event.preventDefault();
                                    document.getElementById('cart-form').submit();" class="cart-btn">
                                <i class="fas fa-shopping-cart"></i>
