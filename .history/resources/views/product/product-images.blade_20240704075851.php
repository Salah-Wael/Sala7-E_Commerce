                        ...
                        <div class="single-product-item">
                            ...
                            @if (Auth::check() && Auth::user()->role == 'admin')
                                ...
                                <a href="{{ route('product.delete.image', $item->id) }}"
                                    ...
                                    document.getElementById('delete-form-{{ $item->image_path }}').submit();" class="cart-btn">
                                <i class="fas fas fa-trash"></i>
                                {{ __('Delete') }}
                                </a>
                                <form id="delete-form-{{ $item->image_path }}" action="{{ route('product.delete.image', $item->id) }}" method="POST" class="d-none">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            @endif
