...
Route::get('/orders', function () {
    ...
    // // Initialize an array to hold the subtotal for each recipient
    // $subTotalPrices = [];

    // // Loop through each recipient and calculate the subtotal for their orders
    // foreach ($orderRecipients as $recipient) {
    //     $subTotalPrices[$recipient->id] = PriceController::subTotal($recipient->orders);
    // };
    // // dd($subTotalPrices);
    // $orderRecipients += $subTotalPrices;
    $orderRecipients->each(function ($recipient) {
        $recipient->subtotal = PriceController::subTotal($recipient->orders);
    });
    return view('orders')->with(['orderRecipients' => $orderRecipients, 'subTotalPrices' => $subTotalPrices]);
})->name('orders');
